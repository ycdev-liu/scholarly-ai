...
class Settings(BaseSettings):
    ...
    # 如果DEFAULT_MODEL为None，将在model_post_init中设置
    DEFAULT_MODEL: AllModelEnum | None = None  # type: ignore[assignment]
    AVAILABLE_MODELS: set[AllModelEnum] = set()  # type: ignore[assignment]

    # 设置OpenAI兼容API，主要用于概念验证
    ...
    DATABASE_TYPE: DatabaseType = (
        DatabaseType.SQLITE
    )  # 选项：DatabaseType.SQLITE 或 DatabaseType.POSTGRES（已弃用：请使用CHECKPOINTER_DB_TYPE和STORE_DB_TYPE代替）
    
    # 为检查点（短期记忆）和存储（长期记忆）分别设置数据库类型
    # 如果未设置，则回退到DATABASE_TYPE以保持向后兼容性
    CHECKPOINTER_DB_TYPE: DatabaseType | None = None  # 检查点的数据库类型（对话历史）
    STORE_DB_TYPE: DatabaseType | None = None  # 存储的数据库类型（长期记忆）
